...
$project_numbers = $this->db->where('agency_id',$company_id)->where('project_number',$project_number)->get('project_imgs')->result();
    ...
    {
        return $this->db->select()->order_by('id','asc')->limit(7)->get('projects')->result();
    }
...
/*end projects and earning*/
    ...
    public function project_view($project_id)
    {
        return $this->db->
        select(
            'u_projects.p_type,u_projects.id as project_id,start_date,end_date,font,quantity,terms_conditions_status,custom_terms_conditions,img_one,img_two,
    ...
    public function all_filling_project($data_type,$limit, $offset,$project_id)
    {
        ...
        $this->db->select('users.id as users_id,first_name,company_email,user_phone,decript_password,projects_title,u_projects.p_type,u_projects.id as project_id,start_date,end_date,font,invoice_type');
        $this->db->from('users');
        $this->db->where('u_projects.p_id',$project_id);
        $this->db->where('users.company_id',$company_id);
        if ($data_type != 'projects') {
