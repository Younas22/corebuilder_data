...
/*end projects and earning*/
    ...
    public function all_filling_project($data_type,$limit, $offset,$project_id)
    {
        ...
        $this->db->select('users.id as users_id,first_name,company_email,profile_img,user_phone,decript_password,projects_title,u_projects.p_type,u_projects.id as project_id,start_date,end_date,font,difficulty,invoice_type,quantity,u_working._right,wrong,earning,refrash_limit');
        $this->db->from('users');
        $this->db->where('u_projects.p_id',$project_id);
        $this->db->where('users.company_id',$company_id);
        if ($data_type != 'projects') {
